...
SELECT DepartmentID, FirstName, LastName FROM Employees WHERE HireDate > (SELECT MIN(HireDate) FROM Employees)
...
SELECT * FROM Employees where NOT Salary = (SELECT MAX(Salary) FROM Employees)
